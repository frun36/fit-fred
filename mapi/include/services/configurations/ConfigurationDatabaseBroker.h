...
class ConfigurationDatabaseBroker : public IndefiniteMapi
{
    ...
    */
    ...
    void connectConstructor(const std::string& operation, QueryConstructor queryConstructor)
    {
    ...
    */
    ...
    void connectExecutor(const std::string& operation, QueryExecutor queryExecutor)
    {
    ...
    */
    ...
    struct InsertParameter {
        ...
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructInsertParameters, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeUpdate, broker, std::placeholders::_1));
        }

        InsertParameter(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                  boardName(tokens[1]),
                                                                  paramName(tokens[2]),
                                                                  value(tokens[3])
        {
    ...
    */
    struct UpdateParameter {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructUpdateParameters, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeUpdate, broker, std::placeholders::_1));
        }

        UpdateParameter(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                  boardName(tokens[1]),
                                                                  paramName(tokens[2]),
                                                                  value(tokens[3])
        {
    ...
    */
    struct SelectParameter {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructSelectParameters, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeSelectParameters, broker, std::placeholders::_1));
        }

        SelectParameter(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                  boardName(tokens[1]),
                                                                  paramName(tokens[2])
        {
        }
    ...
    */
    struct SelectParameterVersion {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructSelectVersionsParameters, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeSelectVerionsParameters, broker, std::placeholders::_1));
        }

        SelectParameterVersion(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                         boardName(tokens[1]),
                                                                         paramName(tokens[2]),
                                                                         startDate(tokens[3])
        {
    ...
    */
    struct SelectConfiguration {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructSelectConfiguration, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeSelectConfiguration, broker, std::placeholders::_1));
        }

        SelectConfiguration(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                      author(tokens[1])
        {
        }
        const std::string& configName;
    ...
    */
    struct InsertConfiguration {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructInsertConfiguration, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeUpdate, broker, std::placeholders::_1));
        }

        InsertConfiguration(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                      author(tokens[1]),
                                                                      comment(tokens[2])
        {
        }
    ...
    */
    struct DeleteParameter {
        ...
        static void Initialize(ConfigurationDatabaseBroker* broker)
        {
            broker->connectConstructor(Command.data(), std::bind(&ConfigurationDatabaseBroker::constructDeleteParameter, broker, std::placeholders::_1));
            broker->connectExecutor(Command.data(), std::bind(&ConfigurationDatabaseBroker::executeUpdate, broker, std::placeholders::_1));
        }

        DeleteParameter(const std::vector<std::string>& tokens) : configName(tokens[0]),
                                                                  boardName(tokens[1]),
                                                                  paramName(tokens[2])
        {
        }
